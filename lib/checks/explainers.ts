...
export const explainers: CheckExplainer[] = [
  ...
  },
  {
    ...
    citationTemplate:
      'One or more Google Ads tags in this GTM container with no Consent Settings configured. Google\'s Consent Mode v2 specification, Google Ads conversion and remarketing tags must honour the ad_storage and ad_user_data consent signals before firing. Without explicit Consent Settings on each tag, the container will send ad measurement data regardless of user choice, creating both regulatory exposure (GDPR, UK GDPR, ePrivacy) and degraded measurement modelling. Fix: configure required additional consent on every Google Ads tag and verify denied/granted/updated paths in Preview before publishing. Source: developers.google.com/tag-platform/security/guides/consent.',
    references: [
      {
  ...
  },
  {
    ...
    directAnswer:
      'duplicate conversion tracking. Either two GTM tags firing the same Google Ads conversion ID and label on the same trigger, or two Google Ads conversion actions configured to count the same business event. Every legitimate conversion is now being reported twice, which inflates conversion volume, doubles conversion value, and feeds Smart Bidding a broken signal.',
    ...
    citationTemplate:
      'Duplicate conversion tracking. Either GTM tags or Google Ads conversion actions counting the same business event more than once. Google\'s conversion tracking documentation, each business event should map to exactly one enabled, Primary conversion action. Duplicate tracking doubles reported conversion volume and value, breaks Smart Bidding signal, and produces dashboards that diverge materially from backend reality. Fix: identify duplicate pairs, consolidate to a single canonical conversion action per event, and annotate the change for historical reporting context. Source: support.google.com/google-ads/answer/6386790.',
    references: [
      {
  ...
  },
  {
    ...
    citationTemplate:
      'This GTM container expects a purchase dataLayer event but the recommended e-commerce shape is incomplete or inconsistent. Google\'s GA4 recommended events specification, conversion tags require a `purchase` event with a nested `ecommerce` object containing `transaction_id`, `value`, `currency`, and an `items` array. Without this structure, downstream Google Ads conversion value, GA4 e-commerce reports, and remarketing audiences are at risk of silent data loss. Fix: standardise the purchase event payload before any GTM tag fires and verify in Preview. Source: developers.google.com/analytics/devguides/collection/ga4/ecommerce.',
    references: [
      {
  ...
  },
  {
    ...
    why: 'Value-based bidding (tROAS, Maximize Conversion Value) is the dominant Google Ads bidding strategy for e-commerce because it lets the algorithm trade conversions for revenue: skip the $10 customer to win the $1,000 customer. The strategy only works if conversion actions carry their actual transaction value. When the value is zero, the algorithm has no information about which conversions to prioritise, falls back to count-based optimisation, and pushes budget toward whatever campaign produces the most cheap conversions. Often at the expense of high-revenue traffic. The check is critical because this shows up in most audits (a 2024 industry survey found this in ~30% of audited e-commerce accounts) and the fix is straightforward but blocked on the data pipeline: the value has to flow from the site to GTM to the conversion tag to Google Ads.',
    howToFix:
  ...
  },
  {
    ...
    directAnswer:
      'The Conversion Linker is in the container, but it is not configured as a setup tag for the Google Ads conversion tags. On a normal page load the linker fires first and the conversion tag picks up the GCLID. But on slower loads, redirect-heavy checkout flows, or consent transitions the order can flip. When that happens, the conversion is sent before the GCLID has been captured, and attribution fails for a subset of users.',
    ...
    citationTemplate:
      'Google Ads conversion tags in this container that share a trigger with the Conversion Linker but do not declare it as a setup tag. Google\'s Tag Sequencing documentation, GTM does not guarantee execution order between tags sharing a trigger; explicit sequencing is required when one tag depends on the side effects of another. Without tag sequencing, conversions can be reported before the GCLID is captured, producing intermittent attribution loss that is invisible in dashboards. Fix: configure each Google Ads conversion tag\'s Tag Sequencing to require the Conversion Linker as a setup tag. Source: support.google.com/tagmanager/answer/6238868.',
    references: [
      {
  ...
  },
  {
    ...
    citationTemplate:
      'Google Ads conversion tags firing on navigation-triggering events (click or form-submit) without configured error handling. Google\'s Tag Manager trigger documentation, the "Wait for Tags" option or an event callback must be configured to prevent the browser navigation from killing in-flight conversion requests. Without these safeguards, conversion data is lost on slower devices and connections in proportion to the latency of the conversion request. Fix: enable "Wait for Tags" with a 2000ms max wait on every flagged trigger and verify completion in Preview under throttled network conditions. Source: support.google.com/tagmanager/answer/7679219.',
    references: [
      {
  ...
  },
  {
    ...
    citationTemplate:
      'This Google Ads account has no enabled Primary conversion action. Google\'s Smart Bidding documentation, value- and volume-based bidding strategies require at least one Primary action to function; without one, automated bidding cannot learn from conversion signal. Fix: immediately mark the canonical macro business outcome as Primary and verify campaign-level goal inheritance. Source: support.google.com/google-ads/answer/12727548.',
    references: [
