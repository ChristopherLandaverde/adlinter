...
export const explainers: CheckExplainer[] = [
  ...
  },
  {
    ...
    why: 'Pinterest documents `value` and `currency` as required for any optimization that bids on revenue, and for any reporting downstream that compares spend to outcome. When Checkout fires without them, the failure cascades.\n\nValue-based bidding can\'t run. Pinterest Smart Bidding objectives that target ROAS need a populated `value` to learn from; without it, the campaign falls back to Checkout-volume optimization and ignores the size of the basket entirely. The Events Manager revenue column reads zero, so the in-platform dashboard contradicts the ecommerce backend on every reconciliation. And the Conversions API mirror inherits the same gap — a server payload that omits `value` can\'t cleanly deduplicate against a browser payload that also omits it.\n\nFor an agency client review, this is the line that fails first. You can defend conversion count. You can\'t defend revenue attribution when `value` is missing from the source event.',
    howToFix:
  ...
  },
  {
    ...
    why: 'PageVisit is the Pinterest equivalent of a base pageview event. Pinterest expects it to fire on every tracked page, and a lot of downstream functionality depends on it being there.\n\nRetargeting collapses without it. The default site-visitor audience in Pinterest is built from PageVisit. No PageVisit, no audience, no retargeting line item. Events Manager match-quality and event-coverage scoring also goes sideways — Pinterest evaluates the tag against a baseline of PageVisit volume, so the diagnostic columns sit empty or show artificially poor coverage on the downstream events. And attribution context weakens: when a Checkout fires without a PageVisit ancestor, Pinterest has less session evidence to weigh against the click-through window, and attribution gets weaker.\n\nThe usual cause is a tag that was installed only on the conversion page (a common pattern when someone copies a Checkout snippet from documentation and stops there), or a GTM trigger set to fire PageVisit only on a single page template instead of All Pages. Sometimes the base tag fires but the PageVisit event call is commented out or guarded by a consent flag that never resolves.',
    howToFix:
  ...
  },
  {
    ...
    why: 'When you install Snap, the base code includes a `snaptr(\'track\', \'PAGE_VIEW\')` call that runs on every page load. That single event does a lot of work: it confirms the pixel is reachable from the user\'s browser, it populates the broadest retargeting audience (all site visitors in the last N days), and it serves as the denominator for funnel diagnostics so Snap can show conversion rates from view to purchase.\n\nWith PAGE_VIEW missing or disabled, all of that breaks. Events Manager shows the pixel as low- or zero-volume and may flag it as unhealthy. Site-visitor retargeting either doesn\'t exist or relies on a custom event with much lower coverage. Funnel reports show conversion rates over an unknown base because Snap doesn\'t know how many pages were viewed.\n\nThe usual cause is one of two things. Either the base code was modified to remove the auto PAGE_VIEW call (often done deliberately for SPAs where developers plan to fire it manually on route changes, then forget the manual call). Or the entire base snippet is gated behind a consent banner that never grants consent because of a misconfiguration.',
    howToFix:
